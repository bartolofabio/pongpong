...
function loop(){
    //verifica se a bola está colidindo com a barra do player1
    if(ball_x >= p1_x && ball_x <= p1_x + 10 && ball_y >= p1_y && ball_y <= p1_y + p_h) {
        ball_x_orientation = 1
    }
    //verifica se a bola está colidindo com a barra do player2 
    else if(ball_x >= p2_x && ball_x <= p2_x + 10 && ball_y >= p2_y && ball_y <= p2_y + p_h) {
        ball_x_orientation = -1
    }

    //verifica a bola bateu no chão ou no teto
...
function writePoints() {
    ...
    // 3* (w/4) = 3/4 de tela = metade da tela do player 2
